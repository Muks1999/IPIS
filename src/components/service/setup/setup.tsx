...
export default class SetupService extends BaseService {
    ...
    async getPlatformNo(platformData: any): Promise<any> {
        const response: any = await this.get(`/setup/platform/${platformData.device}`);
        if (response && response?.data) {
            if(response.data?.error){
                const error = response.data?.error;
            }
    ...
    async getDeviceId(deviceIdData: any): Promise<any> {
        const response: any = await this.get(`/setup/${deviceIdData.device}/${deviceIdData.platform}`);
        if (response && response?.data) {
            if (response.data?.error) {
                const error = response.data?.error;
            }
